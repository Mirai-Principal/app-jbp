...
import { NuevosParticipantesService } from './services/nuevos-participantes.service';
...
@Component({
  selector: 'app-nuevos-participantes',
  imports: [
    Header,
    MatCard,
    MatCardContent,
  ...
  templateUrl: './nuevos-participantes.html',
  styleUrl: './nuevos-participantes.scss',
})
export class NuevosParticipantes {
  // DI
  private sweetAlert = inject(SweetAlertService);
  private nuevosService = inject(NuevosParticipantesService);
  ...
  getParticipantes() {
    this.nuevosService.getParticipantesPorActualizar().subscribe({
      next: (data) => {
  ...
  actualizacionMasivaParticipantes() {
    this.sweetAlert.confirm({
      ...
      if (result) {
        ...
        this.nuevosService.actualizacionMasivaParticipantes().subscribe({
          next: (resp) => {
            console.log(resp);
